...
import MemberModal from './MemberModal'; //모달 컴포넌트 import
...
export default function Member({ peopleInformation }: MemberProps) {
    ...
    const handleMemberClick = (member: PEOPLE_INFORMATION_TYPE) => {
        ...
        <>
            {!loading && ( //loading이 false일 때만 멤버 정보 렌더링
                <S.Container className="mount" isMobile={isMobile}>
                            ...
                            peopleInformation.map((item: PEOPLE_INFORMATION_TYPE, index) => {
                                return (
                                    <S.MemberWrapper key={index}>
                                        {/* Image */}
                                        <S.MemberImgBox onClick={() => handleMemberClick(item)}>
                                            {item.imgSrc ? (
                    ...
                    {selectedMember && <MemberModal member={selectedMember} onClose={handleCloseModal} />}
                </S.Container>
            )}
        </>
    );
